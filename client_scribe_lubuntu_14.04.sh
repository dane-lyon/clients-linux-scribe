...
######### Intégration client scribe 2.3 pour Lubuntu (LXDE) 14.04 LTS #########
# Précision : ce script est en faite identique a celui utilisé pour Xubuntu (compatible).
...
## Autres changements spécifiques à la version LXDE :

# - Installation des paquets libreoffice et vlc (non inclus par défaut dans Lubuntu)
...
#apt-get install -y lubuntu-restricted-addons
#apt-get install -y lubuntu-restricted-extras
